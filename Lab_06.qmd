...
![데이터사이언스 프로세스와 시각화(<https://r4ds.hadley.nz/visualize.html>)](images/visualize.png){#fig-6-1}
...
![그래프의 8대 구성 요소(<https://initiation.doana-r.com/06-ggplot2#1>)](images/graph8factor.png){#fig-ggplot2-layers}
...
`aes()`는 두 곳에 지정할 수 있는데, 최상위 함수인 `ggplot()` 속에 지정하면 '글로벌' 지정이, 특정한 `geom_*()` 속에 지정하면 '로컬' 지정이 된다. 둘의 차이는 뒤에서 살펴본다. [**ggplot2**](https://ggplot2.tidyverse.org/) 패키지가 제공하는 시각속성이 @tbl-aes 에 정리되어 있다.
...
#| fig-cap: "기하객체: geom_point() + geom_smooth() 1"
ggplot(mpg, aes(x = displ, y = hwy, color = class)) + 
  geom_point() + 
  geom_smooth()
```
...
#| fig-cap: "기하객체: geom_point() + geom_smooth() 2"
ggplot(mpg, aes(x = displ, y = hwy)) + 
...
```{r}
#| label: fig-ggplot-variable
#| fig-cap: "ggplot()을 변수에 할당"

p <- ggplot(mpg, aes(x = hwy))
```

```{r}
#| label: fig-ggplot-variable1
#| fig-cap: "변수에 할당된 ggplot() 사용 - 1"

p + geom_density()
```

```{r}
#| label: fig-ggplot-variable2
#| fig-cap: "변수에 할당된 ggplot() 사용 - 2"
...
예를 들어, 전체 데이터로 산점도를 그렸지만, 추세선은 `class`가 `suv`에 속하는 데이터에 대해서만 그리고 싶을 수도 있다. 이러한 경우, `geom_*()`에 `ggplot()`에 넣었던 데이터와 다른 데이터를 넣는 경우, 해당 `geom_*()` 레이어는 `ggplot()` 함수에 지정된 데이터를 따르지 않는다.

```{r}
#| label: fig-ggplot-data
#| fig-cap: "geom_*()에 다른 데이터를 넣은 경우"
ggplot(mpg, aes(x = displ, y = hwy)) +
  geom_point() +
...
[![ColorBrewer(\<http://colorbrewer2.org/\>)](images/colorbrewer2.png){#fig-colorbrewer}](http://colorbrewer2.org/)
...
```{r}
#| label: fig-stat-3
#| fig-cap: "통계적 변환: geom_bar()에 stat = identity 지정"
diamonds |>
  count(cut) |> 
...
```{r}
#| label: fig-stat-5
#| fig-cap: "통계적 변환: geom_bar()에 after_stat() 지정"
...
```{r}
#| label: fig-bar-fill-4
#| fig-cap: "position: fill"
ggplot(mpg, aes(x = drv, fill = class)) + 
...
![**ggplot2** 패키지의 테마 요소(<https://isabella-b.com/blog/ggplot2-theme-elements-reference/>)](images/ggplot2theme.png){#fig-ggplot-theme}
...
```{r}
#| label: fig-ggplot-label-2
#| fig-cap: "라벨링: ggrepel 패키지"
library(ggrepel)
...
Posit에서는 `ggplot2`의 핵심을 간추린 2페이지짜리 [CHEET SHEET](https://posit.co/wp-content/uploads/2022/10/data-visualization-1.pdf)를 제공하고 있다. `ggplot2`를 사용하여 작업할 때, 옆에 켜놓고 작업을 하면 도움이 된다.
