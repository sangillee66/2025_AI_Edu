...
-   데이터 시각화하기(visualize): 데이터에 내재된 패턴, 관계, 분포, 변화, 이상값 등을 시각적 표현으로 변환하여 인간의 인지와 해석을 돕는 과정으로, EDA의 핵심 부분
...
-   시각속성(aesthetics)
    ...
    -   시각속성: 데이터 값의 다양성을 시각적으로 나타내주는 그래픽 요소

        -   컬러(color), 크기(size), 형태(shape), 위치(position)(x-축, y-축), 투명도(alpha) 등

    -   시각속성 매핑(mapping): 특정한 **시각속성**을 특정한 **컬럼**에 부여하는 과정
...
## 기타 구성요소: 통계적 변환(statistical transformation)
